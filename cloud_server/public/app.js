...
(function () {
  ...
  // ─── Live IoT Milking Parlor & Collar Telemetry Stream ───────────────────────
  function renderIotView() {
    return `
      <!-- ESP32 Multi-Sensor Health Collar Stream -->
      <div class="kisan-card" style="border: 2px solid #38bdf8; margin-bottom: 20px;">
        <div class="card-header-flex">
          <div>
            <h2 class="card-title">🐄 DhenuRakshak IoT Collar Telemetry (ESP32 Live)</h2>
            <p class="card-subtitle">Real-time collar telemetry: ADXL345 Jaw Movement / Rumination DSP, LM35 Temperature & NEO-6M GPS</p>
          </div>
          <span class="badge" style="background:#0284c7; color:#fff; padding:6px 12px; border-radius:20px; font-weight:bold;">
            NODE: DHENU-COLLAR-01
          </span>
        </div>

        <!-- Real-Time Sensor Metric Tiles -->
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px; margin: 16px 0;">
          <div class="stat-box" style="background:#0f172a; border-left:4px solid #38bdf8; padding:12px;">
            <span class="stat-label">🐄 Cattle Body Temp</span>
            <span class="stat-value" id="collar_temp" style="font-size:1.6rem; color:#38bdf8;">38.8 °C</span>
            <span class="stat-sub" id="collar_temp_sub" style="color:#4ade80;">Normal Core Temp</span>
          </div>

          <div class="stat-box" style="background:#0f172a; border-left:4px solid #4ade80; padding:12px;">
            <span class="stat-label">🌿 Rumination Chews</span>
            <span class="stat-value" id="collar_cpm" style="font-size:1.6rem; color:#4ade80;">54.2 CPM</span>
            <span class="stat-sub" id="collar_rum_status" style="color:#4ade80;">● ACTIVE RUMINATION</span>
          </div>

          <div class="stat-box" style="background:#0f172a; border-left:4px solid #f59e0b; padding:12px;">
            <span class="stat-label">🦷 Dynamic Jaw Accel</span>
            <span class="stat-value" id="collar_dyn_accel" style="font-size:1.6rem; color:#f59e0b;">0.24 g</span>
            <span class="stat-sub">Chew Amplitude</span>
          </div>

          <div class="stat-box" style="background:#0f172a; border-left:4px solid #ec4899; padding:12px;">
            <span class="stat-label">📍 GPS Live Geo-Tag</span>
            <span class="stat-value" id="collar_gps" style="font-size:1.1rem; color:#ec4899; word-break:break-all;">22.5645, 72.9289</span>
            <span class="stat-sub" id="collar_gps_sub"><a href="https://www.google.com/maps?q=22.5645,72.9289" target="_blank" style="color:#38bdf8; text-decoration:underline;">View OpenStreetMap</a></span>
          </div>
        </div>

        <!-- Canvas for Live Jaw Chewing Oscillation -->
        <div style="margin-top:10px;">
          <div style="display:flex; justify-content:space-between; font-size:0.8rem; color:#94a3b8; margin-bottom:4px;">
            <span>ADXL345 REAL-TIME JAW CHEWING OSCILLATION WAVEFORM (DSP FILTERED)</span>
            <span id="collar_chew_indicator" style="color:#4ade80; font-weight:bold;">CHEWING DETECTED</span>
          </div>
          <div class="iot-canvas-wrap">
            <canvas id="jawWaveformChart" width="800" height="120"></canvas>
          </div>
        </div>
      </div>

      <!-- Automated Milking Parlor Stream -->
      <div class="kisan-card">
        <div class="card-header-flex">
          <div>
          ...
          <button class="btn-primary" id="btnToggleIot">
            ${state.iotActive ? "⏹ " + t("stopStream", "Stop Stream") : "▶ " + t("startStream", "Start Real-Time Stream")}
          </button>
        </div>
        ...
        <!-- Canvas for Live Milking Sensor Waveform -->
        <div class="iot-canvas-wrap">
  ...
  // Ring buffer for jaw waveform
  if (!state.jawWaveform) state.jawWaveform = [];

  function startIotStream() {
    clearInterval(state.iotInterval);
    state.iotInterval = setInterval(async () => {
      try {
        // 1. Fetch Parlor Stream
        const res = await fetch("/api/iot-stream");
        const data = await res.json();
        state.iotHistory.push(data);
        if (state.iotHistory.length > 50) state.iotHistory.shift();

        // 2. Fetch Live Collar Telemetry
        try {
          const collarRes = await fetch("/api/telemetry/live");
          const collarJson = await collarRes.json();
          if (collarJson.success && collarJson.telemetry) {
            const telem = collarJson.telemetry;
            const tempEl = document.getElementById("collar_temp");
            if (tempEl) tempEl.innerText = `${telem.temperature_c.toFixed(1)} °C`;
            
            const tempSub = document.getElementById("collar_temp_sub");
            if (tempSub) {
              if (telem.temperature_c >= 39.8) {
                tempSub.innerText = "🚨 Fever Detected!";
                tempSub.style.color = "#ef4444";
              } else if (telem.temperature_c >= 39.2) {
                tempSub.innerText = "⚠️ Subclinical Rise";
                tempSub.style.color = "#f59e0b";
              } else {
                tempSub.innerText = "Normal Core Temp";
                tempSub.style.color = "#4ade80";
              }
            }

            const cpmEl = document.getElementById("collar_cpm");
            if (cpmEl && telem.jaw_metrics) cpmEl.innerText = `${telem.jaw_metrics.chews_per_minute.toFixed(1)} CPM`;

            const rumEl = document.getElementById("collar_rum_status");
            if (rumEl && telem.jaw_metrics) {
              const isRum = telem.jaw_metrics.rumination_state === "RUMINATING";
              rumEl.innerText = isRum ? "● ACTIVE RUMINATION" : "○ RESTING / GRAZING";
              rumEl.style.color = isRum ? "#4ade80" : "#94a3b8";
            }

            const dynEl = document.getElementById("collar_dyn_accel");
            if (dynEl && telem.jaw_metrics) dynEl.innerText = `${telem.jaw_metrics.dynamic_accel_g.toFixed(3)} g`;

            const gpsEl = document.getElementById("collar_gps");
            if (gpsEl && telem.gps) gpsEl.innerText = `${telem.gps.latitude.toFixed(4)}, ${telem.gps.longitude.toFixed(4)}`;

            const chewInd = document.getElementById("collar_chew_indicator");
            if (chewInd && telem.jaw_metrics) {
              chewInd.innerText = telem.jaw_metrics.is_chewing ? "⚡ CHEW DETECTED" : "— NO CHEW";
              chewInd.style.color = telem.jaw_metrics.is_chewing ? "#38bdf8" : "#64748b";
            }

            // Push to jaw waveform buffer
            if (telem.jaw_metrics) {
              state.jawWaveform.push(telem.jaw_metrics.dynamic_accel_g);
              if (state.jawWaveform.length > 50) state.jawWaveform.shift();
            }
          }
        } catch (err) {
          // Collar stream poll error
        }

        // Update live parlor numbers
        ...
        if (statsDisp) {
          statsDisp.innerHTML = `Accumulated Milk: <strong>${data.flow_rate_lpm ? (data.flow_rate_lpm * 2.4).toFixed(1) : 6.8} L</strong> | Flow: <strong>${data.flow_rate_lpm} L/min</strong> | Temp: <strong>${data.milk_temp_c}°C</strong>`;
        }
  ...
  function drawJawChart() {
    const canvas = document.getElementById("jawWaveformChart");
    if (!canvas) return;
    ...
    if (!state.jawWaveform || state.jawWaveform.length < 2) return;

    // Draw dynamic acceleration chewing waveform
    ctx.strokeStyle = "#38bdf8";
    ctx.lineWidth = 2.5;
    ctx.beginPath();
    state.jawWaveform.forEach((dyn, i) => {
      const x = (i / 50) * w;
      // Map 0.0g - 0.5g dynamic acceleration to canvas height
      const y = h - (Math.min(0.5, dyn) / 0.5) * (h - 10) - 5;
      if (i === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    });
    ctx.stroke();

    // Threshold line (0.20g)
    ctx.strokeStyle = "rgba(239, 68, 68, 0.4)";
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    const threshY = h - (0.20 / 0.5) * (h - 10) - 5;
